...
import SpecUtils._
import zio.test.TestAspect.withLiveEnvironment
...
object SpecUtils {
  ...
  val handlerMessageTrait = new Stateful[Any, Int, Message] {
    override def receive[A](
      ...
      msg match {
        case Str(value) =>
          ZIO.succeed((state + 1, (value + "received plus " + state + 1).asInstanceOf[A]))
      }
  }
...
object RemoteSpec extends ZIOSpecDefault {
  ...
  def spec =
      ...
      suite("Error handling suite")(
        ...
        test("Remote system does not exist") {
          (for {
            ...
            assert(_)(
              fails(isSubtype[ConnectException](anything)) && fails(
                hasField[Throwable, String]("message", _.getMessage, equalTo("Connection refused"))
              )
            )
          )
        },
        test("Remote actor does not exist") {
          (for {
            ...
            assert(_)(
              fails(isSubtype[Throwable](anything)) && fails(
                hasField[Throwable, String]("message", _.getMessage, equalTo("No such remote actor"))
              )
            )
          )
        },
        test("On remote side error message processing error") {
          (for {
            ...
            assert(_)(
              fails(isSubtype[Throwable](anything)) && fails(
                hasField[Throwable, String]("message", _.getMessage, equalTo("Error on remote side"))
              )
            )
          )
        },
    ...
    ).provideCustomLayer(TestConsole.silent ++ ZLayer.fromZIO(testClock))
}
